<?php
	 ...
	 *
	 *  Note: Small modification by Michael King to improve parsing of URLs 
	 *  from filenames. 4/2014
	 */
	 ...
	 class GWTdata
	 {
		 ...
		 *  @param $savepath  String   Optiaonal path to save CSV to (no trailing slash!).
		 */
			public function DownloadCSV($site, $savepath=".")
			{
				if(self::IsLoggedIn() === true) {
					$downloadUrls = self::GetDownloadUrls($site);
					// MK: Changed dash to tilde
		 ...
		 */
			public function DownloadCSV_CrawlErrors($site, $savepath=".", $separated=false)
			{
				if(self::IsLoggedIn() === true) {
					$type_param = "we";
					// MK: changed dash to tilde because URLs can have dashes
					$filename = parse_url($site, PHP_URL_HOST) ."~". date("Ymd~His");
					if($separated) {
